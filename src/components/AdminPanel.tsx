...
const ADMIN_PASSWORD = "wildfall2026";
      ...
      <Modal isOpen={isOpen} onClose={onClose} title="Admin Access">
        <div className="space-y-6 py-4">
          ...
          <p className="text-xs text-gray-500 text-center">
            Default: wildfall2026 (change in AdminPanel.tsx)
          </p>
        </div>
      </Modal>
    );
              ...
              <div>
                ...
                <div className="space-y-3">
                  {factionStats.map(stat => {
                    ...
                    const total = factionStats.reduce((s, f) => s + f.total_points, 0);
                    const percentage = total > 0 ? (stat.total_points / total) * 100 : 0;
                    ...
                    return (
                      <div key={stat.faction} className="bg-gray-900 rounded-sm p-4 border border-white/10">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          {stat.total_operatives} operatives • {Math.round(percentage)}% of total points
                        </p>
                      </div>
                    );
                  })}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="p-4 bg-gray-900 rounded-sm border border-white/10 hover:border-gold transition text-left"
                  >
                    <Package className="w-6 h-6 text-gold mb-2" />
                    <p className="font-bold text-sm">Drop Supply Crate</p>
                    <p className="text-xs text-gray-400">Create timed reward event</p>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <p className="font-bold text-sm">Manage Players</p>
                    <p className="text-xs text-gray-400">Award points, view stats</p>
                  </button>
                </div>
              </div>
            </motion.div>
              ...
              {operatives.map(op => (
                <div key={op.id} className="bg-gray-900 rounded-sm p-4 border border-white/10 flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => {
                        const points = parseInt(prompt('Points to award (use negative to deduct):', '10') || '0');
                  ...
                  <div key={mission.id} className="bg-gray-900 rounded-sm p-3 border border-white/10">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">Week {mission.week} • {mission.completions} completions</p>
                      </div>
                      <button
            ...
            >
              ...
              <div className="bg-gray-900 rounded-sm p-4 border border-white/10">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <div>
                    <label className="text-xs text-gray-400 block mb-1">Min Points</label>
                    <input
                      type="number"
                      value={newCrate.min_points}
                      onChange={(e) => setNewCrate({ ...newCrate, min_points: parseInt(e.target.value) })}
                      className="w-full p-2 bg-black border border-white/10 rounded-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={newCrate.max_points}
                      onChange={(e) => setNewCrate({ ...newCrate, max_points: parseInt(e.target.value) })}
                      className="w-full p-2 bg-black border border-white/10 rounded-sm"
                    />
                  </div>
                </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {crate.current_claims}/{crate.max_claims} claimed • Expires {new Date(crate.expire_time).toLocaleTimeString()}
                          </p>
                        </div>
                      </div>
